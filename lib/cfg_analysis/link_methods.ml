...
let nodes_labels_to_replace (l:label) (n:block node) :(label*block node) list * label list= 
  let new_id id = 
    ...
    Cfg.N_id(int_of_string (Printf.sprintf "%i0%i" i id)) in
  let invokes = ref [] in
  let rec foo (b:block list) id = 
    match b with 
    |[]->[]
    |[x]->
      let new_l = {method_id = l.method_id;node_id=Cfg.N_id(id)} in
      let _ = try List.tl x with Failure(_) -> invokes:=new_l::!invokes; [] in
      let new_n={value = x;next=n.next} in  [(new_l,new_n)]
    |x::y::q->let new_l = {method_id = l.method_id;node_id=Cfg.N_id(id)} in 
      let next_l = {method_id = l.method_id;node_id = new_id id} in 
      let _ = try List.tl x with Failure(_) -> invokes:=new_l::!invokes; [] in
